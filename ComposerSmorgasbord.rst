...
**Step 2.10:** Run the *downloadFabric.sh* script in order to pull the necessary Hyperledger Fabric v1.2.0 images from the Hyperledger project's public Docker Hub repositories::

 bcuser@ubuntu16045:~/composer-tools/packages/fabric-dev-servers$ ./downloadFabric.sh
 
**Step 2.11:** Enter the *docker images* command again and this time you should see that several Hyperledger Fabric v1.2.0 Docker images are available on your system::

 bcuser@ubuntu16045:~/composer-tools/packages/fabric-dev-servers$ docker images
 REPOSITORY                   TAG                 IMAGE ID            CREATED             SIZE
 hyperledger/fabric-ca        1.2.0               7a9a8d2589c9        2 months ago        217MB
 hyperledger/fabric-ccenv     1.2.0               4194dd6c36d9        2 months ago        1.44GB
...
**Step 2.12:** You are about to start your Hyperledger Fabric network.  But before you do that, enter this command to show that you do not currently have any Docker containers running::
 ...
 bcuser@ubuntu16045:~/composer-tools/packages/fabric-dev-servers$ docker ps -a
...
**Step 2.13:** Run the script to start the Hyperledger Fabric network::
 ...
 2018-09-25 14:55:07.795 UTC [channelCmd] InitCmdFactory -> INFO 001 Endorser and orderer connections initialized
 2018-09-25 14:55:07.811 UTC [cli/common] readBlock -> INFO 002 Got status: &{NOT_FOUND}
 2018-09-25 14:55:07.812 UTC [channelCmd] InitCmdFactory -> INFO 003 Endorser and orderer connections initialized
 2018-09-25 14:55:08.013 UTC [cli/common] readBlock -> INFO 004 Received block: 0
 2018-09-25 14:55:08.232 UTC [channelCmd] InitCmdFactory -> INFO 001 Endorser and orderer connections initialized
 2018-09-25 14:55:08.295 UTC [channelCmd] executeJoin -> INFO 002 Successfully submitted proposal to join channel

**Step 2.14:** Now enter *docker ps -a* to see if your Docker containers are running.  They should all be in the *Up* status::

 bcuser@ubuntu16045:~/composer-tools/packages/fabric-dev-servers$ docker ps -a
 CONTAINER ID        IMAGE                                    COMMAND                  CREATED             STATUS              PORTS                                            NAMES
 63b424c60122        hyperledger/fabric-peer:1.2.0       "peer node start"        About a minute ago   Up About a minute   0.0.0.0:7051->7051/tcp, 0.0.0.0:7053->7053/tcp   peer0.org1.example.com
 e86f12b8418c        hyperledger/fabric-ca:1.2.0         "sh -c 'fabric-ca-se…"   About a minute ago   Up About a minute   0.0.0.0:7054->7054/tcp                           ca.org1.example.com
 4e954d94b95a        hyperledger/fabric-couchdb:0.4.10   "tini -- /docker-ent…"   About a minute ago   Up About a minute   4369/tcp, 9100/tcp, 0.0.0.0:5984->5984/tcp       couchdb
 25c5719433ae        hyperledger/fabric-orderer:1.2.0    "orderer"                About a minute ago   Up About a minute   0.0.0.0:7050->7050/tcp                           orderer.example.com
...
**Step 2.18:** Run this Docker command and you will see that a new Docker image was created for the Hyperledger Composer business network that you just deployed::
 ...
 REPOSITORY                                                                                                                   TAG                 IMAGE ID            CREATED              SIZE
 dev-peer0.org1.example.com-digitalproperty-network-0.2.5-d8060b1a22e5bca07604169f2547a96dedad6f2f092216599fe40995cbc32dea   latest              0383dead5025        2 minutes ago       1.56GB
...
**Step 2.19:** Run the Docker command to show your Docker containers and you will see that a Docker container based on your new Docker image has been created (it should be the first container listed in the output)::
 ...
 CONTAINER ID        IMAGE                                                                                                                        COMMAND                  CREATED             STATUS              PORTS                                            NAMES
 02ed703722bd        dev-peer0.org1.example.com-digitalproperty-network-0.2.5-d8060b1a22e5bca07604169f2547a96dedad6f2f092216599fe40995cbc32dea   "/bin/sh -c 'cd /usr…"   2 minutes ago       Up 2 minutes                                                         dev-peer0.org1.example.com-digitalproperty-network-0.2.5
 63b424c60122        hyperledger/fabric-peer:1.2.0                                                                                               "peer node start"        10 minutes ago      Up 10 minutes       0.0.0.0:7051->7051/tcp, 0.0.0.0:7053->7053/tcp   peer0.org1.example.com
 e86f12b8418c        hyperledger/fabric-ca:1.2.0                                                                                                 "sh -c 'fabric-ca-se…"   10 minutes ago      Up 10 minutes       0.0.0.0:7054->7054/tcp                           ca.org1.example.com
 4e954d94b95a        hyperledger/fabric-couchdb:0.4.10                                                                                           "tini -- /docker-ent…"   10 minutes ago      Up 10 minutes       4369/tcp, 9100/tcp, 0.0.0.0:5984->5984/tcp       couchdb
 25c5719433ae        hyperledger/fabric-orderer:1.2.0                                                                                            "orderer"                10 minutes ago      Up 10 minutes       0.0.0.0:7050->7050/tcp                           orderer.example.com
...
**Step 4.35:** In order to get the changes you made in the last section, which are in the Business Network Archive (BNA) that you exported, two steps are required- a *composer network install* which reads the exported BNA and installs its definitions onto the Fabric peer, and then a *composer network upgrade* which will create a new chaincode image containing these updates, and then start a container based on this image.  Perform the first step::
 ...
 bcuser@ubuntu16045:~/composer-sample-applications/packages/digitalproperty-app$ composer network install -a ~/modified-digitalproperty-network.bna --card PeerAdmin@hlfv1
 ✔ Installing business network. This may take a minute...
...
**Step 4.36:** Now run the *composer network upgrade* command.  If your version differs from *0.2.6-deploy.2* use the value shown on your system in place of *0.2.6-deploy.2* in the command::
 ...
 bcuser@ubuntu16045:~/composer-sample-applications/packages/digitalproperty-app$ composer network upgrade -n digitalproperty-network -V 0.2.6-deploy.2 -c PeerAdmin@hlfv1
...
**Step 4.37:** You can see that a new Docker image was created for the updated business network-  observe the first image listed in the output and see that its version name, *0.2.6-deploy.2* is part of the image name::
 ...
 bcuser@ubuntu16045:~/composer-sample-applications/packages/digitalproperty-app$ docker images dev-*
 REPOSITORY                                                                                                                           TAG                 IMAGE ID            CREATED             SIZE
 dev-peer0.org1.example.com-digitalproperty-network-0.2.6-deploy.2-3acdcbff3f4e90cad8a30b395f0d5b8da1db04b68e0b903b75acf52f1148de08   latest              ffd3ae917dde        About a minute ago   1.56GB
 dev-peer0.org1.example.com-digitalproperty-network-0.2.5-d8060b1a22e5bca07604169f2547a96dedad6f2f092216599fe40995cbc32dea            latest              0383dead5025        About an hour ago    1.56GB

**Step 4.38:** Similary, you can see that a new Docker container has been created for the updated business network::

 bcuser@ubuntu16045:~/composer-sample-applications/packages/digitalproperty-app$ docker ps -a
 CONTAINER ID        IMAGE                                                                                                                                COMMAND                  CREATED             STATUS              PORTS                                            NAMES
 66b452310db1        dev-peer0.org1.example.com-digitalproperty-network-0.2.6-deploy.2-3acdcbff3f4e90cad8a30b395f0d5b8da1db04b68e0b903b75acf52f1148de08   "/bin/sh -c 'cd /usr…"   About a minute ago   Up About a minute                                                    dev-peer0.org1.example.com-digitalproperty-network-0.2.6-deploy.2
 02ed703722bd        dev-peer0.org1.example.com-digitalproperty-network-0.2.5-d8060b1a22e5bca07604169f2547a96dedad6f2f092216599fe40995cbc32dea            "/bin/sh -c 'cd /usr…"   About an hour ago    Up About an hour                                                     dev-peer0.org1.example.com-digitalproperty-network-0.2.5
 63b424c60122        hyperledger/fabric-peer:1.2.0                                                                                                        "peer node start"        About an hour ago    Up About an hour    0.0.0.0:7051->7051/tcp, 0.0.0.0:7053->7053/tcp   peer0.org1.example.com
 e86f12b8418c        hyperledger/fabric-ca:1.2.0                                                                                                          "sh -c 'fabric-ca-se…"   About an hour ago    Up About an hour    0.0.0.0:7054->7054/tcp                           ca.org1.example.com
 4e954d94b95a        hyperledger/fabric-couchdb:0.4.10                                                                                                    "tini -- /docker-ent…"   About an hour ago    Up About an hour    4369/tcp, 9100/tcp, 0.0.0.0:5984->5984/tcp       couchdb
 25c5719433ae        hyperledger/fabric-orderer:1.2.0                                                                                                     "orderer"                About an hour ago    Up About an hour    0.0.0.0:7050->7050/tcp                           orderer.example.com
...
Hyperledger Fabric v1.2.0 network instead of working in Web Browser mode- this way you don't have to hassle with exporting the changes made in Web Browser mode to a file and then importing those changes into Hyperledger Fabric, like you did in an *Section 4*.
